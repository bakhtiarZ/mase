...
def aligned_generate_lookup(data_width: int, f_width: int, function: str, type="hex"):
    ...
    while pi <= maxval:
        ...
        lut[doubletofx(data_width=data_width, f_width=f_width, num=pi, type=type)] = (
            doubletofx(
                data_width=data_width, f_width=f_width, num=val.item(), type=type
            )
        )
...
def lookup_to_file(data_width: int, f_width: int, function: str, file_path=None):
    dicto = aligned_generate_lookup(
        data_width=data_width, f_width=f_width, function=function, type="hex"
    )
    dicto = {
        k: v for k, v in dicto.items() if k not in ["data_width", "f_width", "func"]
    }
...
lookup_to_file(
    ...
    4,
    "silu",
    "/workspace/machop/mase_components/activations/rtl/hex_aligned_silu_map.mem",
)
